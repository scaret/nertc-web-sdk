...
export function JSONParse(options?: Options) {
  ...
  const number = function () {
    ...
    if (!isFinite(number)) {
      ...
      if (string.length > 15) {
        return _options.storeAsString ? string : _options.useNativeBigInt ? BigInt(string) : null
      }
      return !_options.alwaysParseAsBig ? number : _options.useNativeBigInt ? BigInt(number) : null
    }
  }
  function string() {
