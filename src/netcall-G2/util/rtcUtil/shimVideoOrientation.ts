...
export function canShimVideoOrientation(offer: { sdp: string }, answer: { sdp: string }) {
  ...
  if (
    videoOrientationMatch &&
    offer.sdp.indexOf(marker) > -1 &&
    answer.sdp.indexOf(videoOrientationMatch[1]) === -1
  ) {
    return true
  } else {
    return false
...
export function shimVideoOrientation(offer: { sdp: string }, answer: { sdp: string }) {
  ...
  if (
    ...
    console.log(
      `shimVideoOrientation for ${getBrowserInfo().browserName} ${
    ...
    console.error(
      'Failed to shimVideoOrientation',
      videoOrientationMatch,
      offer.sdp.indexOf(marker),
      videoOrientationMatch && answer.sdp.indexOf(videoOrientationMatch[1])
    )
  }
}
