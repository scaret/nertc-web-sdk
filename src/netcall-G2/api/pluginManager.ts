...
class PluginManager extends EventEmitter {
      ...
      for (let pathToPlugin in this.plugins) {
        ...
        if (plugin && plugin.installed && plugin.module.extendClient) {
          try {
            ...
            client.logger.log(
              `成功加载插件 client-created ${plugin.module.name}@${plugin.module.version}。花费 ${
                Date.now() - start
              }`
            )
          } catch (e: any) {
            client.logger.error(
              `插件无法加载 client-created ${plugin.module.name} ${e.name} ${e.message}`
            )
          }
        }
      }
      ...
      for (let pathToPlugin in this.plugins) {
        ...
        if (plugin && plugin.installed && plugin.module.extendLocalStream) {
          try {
            ...
            localStream.logger.log(
              `成功加载插件 stream-created ${plugin.module.name}。花费 ${Date.now() - start}`
            )
          } catch (e: any) {
            localStream.logger.error(
              `插件无法加载 stream-created ${plugin.module.name} ${e.name} ${e.message}`
            )
          }
        }
      }
  ...
  async import(pathToPlugin: string, install = true) {
    if (this.plugins[pathToPlugin]) {
      const plugin = this.plugins[pathToPlugin]
      this.logger.log(
        `import:找到已下载插件 ${plugin.module.name}@${plugin.module.version}【${plugin.module.description}】 ${pathToPlugin}`
      )
    } else {
    ...
    if (install && !plugin.installed) {
      this.logger.log(
        `import:成功下载插件 ${plugin.module.name}@${plugin.module.version}【${plugin.module.description}】开始安装。。。`
      )
  ...
  async install(pathToPlugin: string, NERTC: any) {
    ...
    try {
      ...
      this.logger.log(
        `成功安装插件 ${plugin.module.name}@${plugin.module.version}【${plugin.module.description}】`
      )
    } catch (e: any) {
      this.logger.error(
        `无法安装插件 ${plugin.module.name}@${plugin.module.version}【${plugin.module.description}`,
        e.name,
        e.message,
        e.stack
      )
