...
class FormativeStatsReport {
  ...
  getPacketLossRate(prev: PacketLostData, next: PacketLostData, isSend = false) {
    ...
    let lostRate = isSend
      ? parseFloat((((nextLost - prevLost) / (nextRecv - prevRecv)) * 100).toFixed(1))
      : parseFloat(
          (((nextLost - prevLost) / (nextLost - prevLost + (nextRecv - prevRecv))) * 100).toFixed(1)
        )
    return lostRate
  }
    ...
    for (let i in data) {
      ...
      if (i.indexOf('_send_') !== -1 && i.indexOf('_audioSlave') !== -1) {
        if (this.paramSecond.upAudioSlaveCache[uid]) {
          ...
          currentData.send.audioSlave.prevPacket =
            this.paramSecond.upAudioSlaveCache[uid].packetsSent
          data[i].alr = this.getPacketLossRate(
            this.paramSecond.upAudioSlaveCache[uid],
            data[i],
            true
          )
          this.dispatchExceptionEventSendAudio(
            this.paramSecond.upAudioSlaveCache[uid],
            data[i],
            uid
          )
        ...
        currentData.send.audioSlave.jitter = currentData.recv.audioSlave.jitter =
          data[i].googJitterReceived || 0
        ...
        currentData.send.audio.jitter = currentData.recv.audio.jitter =
          data[i].googJitterReceived || 0
        ...
        data[i] = Object.assign({}, data['bweforvideo'], data[i]) //FIXME 多条视频流时BWE是针对所有视频的。
        bytesSent += parseInt(data[i].bytesSent)
        ...
        currentData.send.video.jitter = currentData.recv.video.jitter =
          data[i].googJitterReceived || 0
        ...
        currentData.send.screen.jitter = currentData.recv.screen.jitter =
          data[i].googJitterReceived || 0
        ...
        if (this.paramSecond.downAudioSlaveCache[uid]) {
          ...
          currentData.recv.audio.prevPacket =
            this.paramSecond.downAudioSlaveCache[uid].packetsReceived
          data[i].alr = this.getPacketLossRate(this.paramSecond.downAudioSlaveCache[uid], data[i])
        }
        let stats = this.getRemoteAudioFreezeStats(
          this.paramSecond.downAudioSlaveCache[uid],
          data[i],
          uid
        )
        data[i].freezeTime = stats.freezeTime
        ...
        if (
          ...
          parseInt(data[i].googDecodingNormal) > 0
        ) {
        ...
        if (
          !this.firstData.recvFirstData[uid].recvFirstAudioPackage &&
          parseInt(data[i].packetsReceived) > 0
        ) {
        ...
        let stats = this.getRemoteAudioFreezeStats(
          this.paramSecond.downAudioCache[uid],
          data[i],
          uid
        )
        data[i].freezeTime = stats.freezeTime
        ...
        if (
          !this.firstData.recvFirstData[uid].recvFirstVideoPackage &&
          data[i].packetsReceived > 0
        ) {
        ...
        let stats = this.getRemoteVideoFreezeStats(
          this.paramSecond.downVideoCache[uid],
          data[i],
          uid
        )
        data[i].freezeTime = stats.freezeTime
        ...
        if (
          !this.firstData.recvFirstData[uid].recvFirstScreenPackage &&
          data[i].packetsReceived > 0
        ) {
        ...
        let stats = this.getRemoteScreenFreezeStats(
          this.paramSecond.downScreenCache[uid],
          data[i],
          uid
        )
        data[i].freezeTime = stats.freezeTime
        ...
        if (suid == this.adapterRef.channelInfo.uid) {
          this.adapterRef.transportStats.OutgoingAvailableBandwidth =
            data[i].availableOutgoingBitrate / 1000
        }
      } else {
  ...
  // 组装下行的媒体信息
  updateRxMediaInfo(
    downAudioList: DownAudioItem[],
    downAudioSlaveList: DownAudioItem[],
    downVideoList: DownVideoItem[],
    downScreenList: DownVideoItem[]
  ) {
    ...
    downAudioSlaveList.map((item) => {
      ...
      this.adapterRef.remoteAudioSlaveStats[uid] = {
        ...
        MuteState:
          remoteStream &&
          (remoteStream.muteStatus.audio.send || remoteStream.muteStatus.audio.recv),
    ...
    downVideoList.map((item) => {
      ...
      RecvBitrate = RecvBitrate + item.bitsReceivedPerSecond
      video2.v_res.push(
        (item.googFrameWidthReceived || 0) + 'x' + (item.googFrameHeightReceived || 0)
      )
    ...
    downScreenList.map((item) => {
      ...
      this.adapterRef.remoteScreenStats[uid] = {
        ...
        MuteState:
          remoteStream &&
          (remoteStream.muteStatus.screen.send || remoteStream.muteStatus.screen.recv), //(remoteStream && remoteStream.screen) || (remoteStream && remoteStream.muteStatus.screen),
        PacketLossRate: item.vlr || 0,
        RecvBitrate: item.bitsReceivedPerSecond || 0,
        RecvResolutionHeight: parseInt(item.googFrameHeightReceived) || 0,
        RecvResolutionWidth: parseInt(item.googFrameWidthReceived) || 0,
        ...
        TotalPlayDuration:
          (this.firstData.recvFirstData[uid] &&
            this.firstData.recvFirstData[uid].screenTotalPlayDuration) ||
          (screenDom && screenDom.played && screenDom.played.length ? screenDom.played.end(0) : 0),
        TransportDelay: parseInt(item.googCurrentDelayMs) || 0
      }
  ...
  getLocalMediaStats(
    upAudioList: UpAudioItem[],
    upAudioSlaveList: UpAudioItem[],
    upVideoList: UpVideoItem[],
    upScreenList: UpVideoItem[]
  ) {
    let result = {
      ...
      real_v_res:
        ((upVideoList[0] && upVideoList[0].googFrameWidthSent) || 0) +
        'x' +
        ((upVideoList[0] && upVideoList[0].googFrameHeightSent) || 0),
      real_s_res:
        ((upScreenList[0] && upScreenList[0].googFrameWidthSent) || 0) +
        'x' +
        ((upScreenList[0] && upScreenList[0].googFrameHeightSent) || 0),
      ...
      a_cap_volume:
        (this.adapterRef.localStream &&
          Math.round(this.adapterRef.localStream.getAudioLevel() * 32768)) ||
        0, // 客户端采集的声音声量，int16
    ...
    if (
      ...
      this.adapterRef.localStream &&
      this.adapterRef.localStream.audioProfile == 'speech_standard'
    ) {
      SamplingRate = 32
    }
    ...
    this.adapterRef.localScreenStats[0] = {
      ...
      CaptureResolutionHeight:
        parseInt(upScreenList[0] && upScreenList[0].googFrameHeightInput) || 0,
  ...
  dispatchExceptionEventRecvAudio(prev: DownAudioItem, next: DownAudioItem, uid: number | string) {
    ...
    let audioDecodingNormalDelta =
      parseInt(next.googDecodingNormal) - parseInt(prev.googDecodingNormal)
  ...
  getRemoteAudioFreezeStats(prev: DownAudioItem, next: DownAudioItem, uid: number | string) {
    ...
    let prevStuck =
      parseInt(prev.googDecodingPLC) +
      parseInt(prev.googDecodingCNG) +
      parseInt(prev.googDecodingPLCCNG)
    let prevNormal = parseInt(prev.googDecodingCTN)

    let nextStuck =
      parseInt(next.googDecodingPLC) +
      parseInt(next.googDecodingCNG) +
      parseInt(next.googDecodingPLCCNG)
