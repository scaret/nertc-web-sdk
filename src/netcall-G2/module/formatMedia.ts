...
import { Timer, AdapterRef, FormatMediaOptions, ClientRecordConfig, ILogger } from '../types'
...
class FormatMedia extends EventEmitter{
    ...
    this.canvasTimer = setInterval(() => {
      ...
      switch(videoDoms.length) {
        ...
        default:
          ...
          this.drawSingleVideo(videoDoms[4], oneThirdWidht, oneThirdHeight, oneThirdWidht, oneThirdHeight)
          this.drawSingleVideo(videoDoms[5], oneThirdWidht * 2, oneThirdHeight, oneThirdWidht, oneThirdHeight)
          this.drawSingleVideo(videoDoms[6], 0, oneThirdHeight * 2, oneThirdWidht, oneThirdHeight)
          this.drawSingleVideo(videoDoms[7], oneThirdWidht, oneThirdHeight * 2, oneThirdWidht, oneThirdHeight)
          this.drawSingleVideo(videoDoms[8], oneThirdWidht * 2, oneThirdHeight * 2, oneThirdWidht, oneThirdHeight)
          break;
      }
  ...
  drawSingleVideo(videoDom: HTMLVideoElement|undefined, containerLeft:number, containerTop: number, containerWidth: number, containerHeight: number){
    if (!videoDom){
      return;
    }
    // 不要拉伸，留出黑边。
    const zoomRatio = Math.min(containerWidth / videoDom.videoWidth, containerHeight / videoDom.videoHeight)
