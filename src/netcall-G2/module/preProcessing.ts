    ...
    if (preProcessing) {
      if (preProcessing.videoElem.videoWidth && preProcessing.videoElem.videoHeight) {
        if (
          ...
          mediaHelper.logger.warn(
            `前处理宽高变化 ${mediaType} ${preProcessing.canvasElem.width}x${preProcessing.canvasElem.height} => ${preProcessing.videoElem.videoWidth}x${preProcessing.videoElem.videoHeight}`
          )
...
export async function disablePreProcessing(
  mediaHelper: MediaHelper,
  mediaType: 'video' | 'screen' = 'video',
  keepFlag = false
) {
  ...
  // 0. 去除最后一帧
  preProcessing.canvasCtx.clearRect(
    0,
    0,
    preProcessing.canvasElem.width,
    preProcessing.canvasElem.height
  )
  ...
  if (sender) {
    if (videoTrack?.readyState === 'live') {
      ...
      mediaHelper.logger.log(
        `disablePreProcessing ${mediaType} 成功替换上行为【${videoTrack.label}】`
      )
    } else {
...
export function preProcessingSyncState(
  mediaHelper: MediaHelper,
  mediaType: 'video' | 'screen',
  ...
  if (config.videoTrack) {
    if (config.videoTrack.enabled !== config.canvasTrack.enabled) {
      mediaHelper.logger.log(
        `preProcessingCopy: 更改mute状态 ${mediaType} ${config.canvasTrack.enabled} => ${config.videoTrack.enabled}`
      )
...
export function preProcessingCopy(
  mediaHelper: MediaHelper,
  mediaType: 'video' | 'screen',
  config: PreProcessingConfig
) {
...
export function preProcessingPureColor(
  mediaHelper: MediaHelper,
  mediaType: 'video' | 'screen',
  config: PreProcessingConfig
) {
