...
import { createAttributeBuffer, parseAttributes, TypedArray } from './buffer-attribute'
...
export class Program {
   ...
   */
  updateUniform(name: string, updateHandler: (value: UniformValueType) => void | UniformValueType) {
   ...
   */
  updateAttribute(name: string, updateHandler: (typedArray: TypedArray) => void | TypedArray) {
  ...
  setIndices(attrBuffer: ReturnType<typeof createAttributeBuffer>) {
    if (attrBuffer && attrBuffer.name === 'indices') {
      ...
      gl.bufferData(attrBuffer.target, attrBuffer.typedArray, attrBuffer.usage)
    }
