...
function getTarget(gl: WebGLRenderingContext, target?: 'ARRAY_BUFFER' | 'ELEMENT_ARRAY_BUFFER') {
  return {
...
function getAttrDataType(gl: WebGLRenderingContext, typedArray: TypedArray) {
  ...
  const typeStr = Object.prototype.toString.call(typedArray).replace(/object|\[|\]|\s/g, '')
 ...
 */
export function parseAttributes(gl: WebGLRenderingContext, program: WebGLProgram) {
  ...
  const attributes: {
    [key: string]: {
      ...
      bufferSetter: (attrBuffer: ReturnType<typeof createAttributeBuffer>) => void
        ...
        const attri: typeof attributes[string] = {
          ...
          setter: (typedArray?: TypedArray) => {
            if (typedArray) {
              const { buffer } = attri
              if (!buffer) {
                console.error(`buffer of attribute:[${name}] is not initialized.`)
                return
              }
              const { type } = attri
              if (getAttrDataType(gl, typedArray) !== type) {
                console.error(`typedArray's type of attribute:[${name}] is inconsistent.`)
                return
              }
              attri.typedArray = typedArray
              const { itemSize, normalized, target, usage, stride, offset } = attri
              gl.bindBuffer(target, buffer)
              gl.bufferData(target, typedArray, usage)
              gl.enableVertexAttribArray(location)
              gl.vertexAttribPointer(location, itemSize, type, normalized, stride, offset)
            } else {
              const { buffer, type, itemSize, normalized, target, stride, offset } = attri
              gl.bindBuffer(target, buffer)
              gl.vertexAttribPointer(location, itemSize, type, normalized, stride, offset)
            }
          },
