...
function genUniformSetter(
  ...
  if (type === gl.FLOAT) {
    return isArray
      ? (value: Float32List) => gl.uniform1fv(location, value)
      : (value: number) => gl.uniform1f(location, value)
  }
  const floatVecMap = {
    [gl.FLOAT_VEC2]: (value: Float32List) => gl.uniform2fv(location, value),
    [gl.FLOAT_VEC3]: (value: Float32List) => gl.uniform3fv(location, value),
    [gl.FLOAT_VEC4]: (value: Float32List) => gl.uniform4fv(location, value)
  }
  ...
  if (type === gl.INT || type === gl.BOOL) {
    return isArray
      ? (value: Int32List) => gl.uniform1iv(location, value)
  ...
  const intVecMap = {
    [gl.INT_VEC2]: (value: Int32List) => gl.uniform2iv(location, value),
    [gl.INT_VEC3]: (value: Int32List) => gl.uniform3iv(location, value),
    [gl.INT_VEC4]: (value: Int32List) => gl.uniform4iv(location, value)
  }
  const intVecFn = intVecMap[boolTypeMap[type] ?? type]
  if (intVecFn) return intVecFn

  const floatMatMap = {
    [gl.FLOAT_MAT2]: (value: Float32List) => gl.uniformMatrix2fv(location, false, value),
    [gl.FLOAT_MAT3]: (value: Float32List) => gl.uniformMatrix3fv(location, false, value),
    [gl.FLOAT_MAT4]: (value: Float32List) => gl.uniformMatrix4fv(location, false, value)
  }
 ...
 */
export function parseUniforms(gl: WebGLRenderingContext, program: WebGLProgram) {
  ...
  for (let i = 0; i < nums; i++) {
    ...
    if (uniform) {
      ...
      const name = isArray ? uniform.name.replace('[0]', '') : uniform.name
