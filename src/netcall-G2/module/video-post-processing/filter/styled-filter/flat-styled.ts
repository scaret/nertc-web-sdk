...
export class FlatStyled extends StyledFilter {
  ...
  private initStyled() {
    ...
    for (let i = 0; i < this.smCount; i++) {
      ...
      smProgram.setUniform(
        'map',
        i === 0 ? this.map : this.framebuffers[`sm${i - 1}`].targetTexture
      )
    ...
    mipProgram.setUniform('map', this.framebuffers[`sm${this.smCount - 1}`].targetTexture)
    ...
    flatProgram.setUniform('map', this.framebuffers[`sm${this.smCount - 1}`].targetTexture)
  ...
  updateSize() {
    ...
    for (let i = 0; i < this.smCount; i++) {
      this.programs[`sm${i}`]?.setUniform('size', [size.width, size.height])
