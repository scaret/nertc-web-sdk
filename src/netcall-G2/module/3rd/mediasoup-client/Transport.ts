  ...
  async prepareLocalSdp(kind: 'video' | 'audio', edgeRtpCapabilities: any, uid: number | string) {
    try {
      ...
      if (
        ...
        let extendedRtpCapabilities = ortc.getExtendedRtpCapabilities(
          rtpCapabilities,
          edgeRtpCapabilities
        )
        // Generate our receiving RTP capabilities for receiving media.
        this._recvRtpCapabilities = ortc.getRecvRtpCapabilities(extendedRtpCapabilities)
  ...
  updateSenderInfo(
    ...
    if (!record) {
      ...
      if (
        ...
        Logger.debug(
          prefix,
          `Sender Change. mediaType:${record.mediaType} => ${mediaType}, StreamType: ${record.streamType} => ${streamType}, LocalId: ${record.localId} => ${localId} index: ${record.index}`
        )
        record.mediaType = mediaType
  ...
  updateReceiverInfo(
    ...
    if (!record) {
      ...
      if (record.uid !== uid || record.mediaType !== mediaType || record.localId !== localId) {
        Logger.debug(
          prefix,
          `Receiver Change. uid:${record.uid} => ${uid}, mediaType:${record.mediaType} => ${mediaType} localId: ${record.localId} => ${localId} index: ${record.index}`
        )
        record.uid = uid
        record.mediaType = mediaType
        record.localId = localId
    ...
    return this._awaitQueue.push(async () => {
      ...
      if (rtpReceiver) {
        ...
        if (rtpReceiver.track.kind === 'audio' && getParameters().audioPlayoutDelayHint) {
          //@ts-ignore
          rtpReceiver.playoutDelayHint = getParameters().audioPlayoutDelayHint
          Logger.debug(prefix, 'audioPlayoutDelayHint: ', getParameters().audioPlayoutDelayHint)
        }
        if (rtpReceiver.track.kind === 'video' && getParameters().videoPlayoutDelayHint) {
          //@ts-ignore
          rtpReceiver.playoutDelayHint = getParameters().videoPlayoutDelayHint
          Logger.debug(prefix, 'videoPlayoutDelayHint: ', getParameters().videoPlayoutDelayHint)
        }
      }
