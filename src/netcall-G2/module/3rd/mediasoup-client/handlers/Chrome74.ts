...
export class Chrome74 extends HandlerInterface {
  ...
  async restartIce(iceParameters: IceParameters): Promise<void> {
    ...
    if (this._direction /*=== 'send'*/) {
      ...
      Logger.debug(prefix, 'restartIce() | calling pc.setRemoteDescription() [answer]')
  ...
  async send({
    ...
    if (appData.mediaType === 'audio' && this._pc.audioSender) {
      ...
      Logger.debug(
        prefix,
        'audioSlaveSender 更新 track: ',
        this._pc.audioSlaveSender.track,
        '=>',
        track
      )
    ...
    if (!localId) {
      throw new RtcError({
        ...
        message: `Chrome.send: localId 未找到`
      })
    }
  ...
  async getSenderStats(localId: string): Promise<RTCStatsReport> {
    ...
    if (!transceiver) {
      throw new RtcError({
        code: ErrorCode.UNKNOWN_TYPE_ERROR,
        message: `Chrome.getSenderStats: RTCRtpTransceiver 未找到`
      })
    }
  ...
  async stopReceiving(localId: string): Promise<void> {
    ...
    if (!transceiver || !transceiver.mid) {
      throw new RtcError({
        code: ErrorCode.UNKNOWN_TYPE_ERROR,
        message: `Chrome.stopReceiving: RTCRtpTransceiver 未找到`
      })
    }
    if (
  ...
  async getReceiverStats(localId: string): Promise<RTCStatsReport> {
    ...
    if (!transceiver) {
      throw new RtcError({
        code: ErrorCode.UNKNOWN_TYPE_ERROR,
        message: `Chrome.getReceiverStats: RTCRtpTransceiver 未找到`
      })
    }
