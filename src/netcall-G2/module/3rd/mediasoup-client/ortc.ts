 ...
 * It throws if invalid.
 */
export function validateRtpHeaderExtensionParameters(ext: RtpHeaderExtensionParameters): void {
  if (typeof ext !== 'object') throw new TypeError('ext is not an object')
...
export function getExtendedRtpCapabilities(
  ...
  for (const remoteCodec of remoteCaps.codecs || []) {
    ...
    const matchingLocalCodec = (localCaps.codecs || []).find((localCodec: RtpCodecCapability) =>
  ...
  for (const extendedCodec of extendedRtpCapabilities.codecs) {
    ...
    const matchingRemoteRtxCodec = remoteCaps.codecs!.find(
      (remoteCodec: RtpCodecCapability) =>
        isRtxCodec(remoteCodec) && remoteCodec.parameters.apt === extendedCodec.remotePayloadType
    )
  ...
  for (const remoteExt of remoteCaps.headerExtensions!) {
    const matchingLocalExt = localCaps.headerExtensions!.find((localExt: RtpHeaderExtension) =>
...
export function getRecvRtpCapabilities(extendedRtpCapabilities: any): RtpCapabilities {
  ...
  for (const extendedCodec of extendedRtpCapabilities.codecs) {
    ...
    // Add RTX codec.
    if (!extendedCodec.localRtxPayloadType) continue
...
export function getSendingRemoteRtpParameters(
  ...
  if (
    ...
    for (const codec of rtpParameters.codecs) {
      codec.rtcpFeedback = (codec.rtcpFeedback || []).filter(
        (fb: RtcpFeedback) => fb.type !== 'transport-cc' && fb.type !== 'goog-remb'
      )
    }
  }
...
export function canSend(kind: MediaKind, extendedRtpCapabilities: any): boolean {
  return extendedRtpCapabilities.codecs.some((codec: any) => codec.kind === kind)
}
 ...
 * capabilities.
 */
export function canReceive(rtpParameters: RtpParameters, extendedRtpCapabilities: any): boolean {
  // This may throw.
  ...
  for (let i in rtpParameters.codecs) {
    ...
    for (let j in extendedRtpCapabilities.codecs) {
      if (
        extendedRtpCapabilities.codecs[j].localPayloadType === rtpParameters.codecs[i].payloadType
      ) {
        return true
      }
    }
...
function matchHeaderExtensions(aExt: RtpHeaderExtension, bExt: RtpHeaderExtension): boolean {
