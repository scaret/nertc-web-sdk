...
class DeviceManager extends RTCEventEmitter {
    ...
    devices.forEach(function (device) {
      if (options.videoinput && device.kind === 'videoinput') {
        let deviceInfo: DeviceInfo = {
          ...
          label:
            device.label ||
            (options.noFillLabel ? device.label : 'camera ' + (result.video.length + 1))
        }
        if (options.groupId) {
        ...
        deviceInfo = {
          ...
          label:
            device.label ||
            (options.noFillLabel ? device.label : 'microphone ' + (result.audioIn.length + 1))
        }
        if (options.groupId) {
          deviceInfo.groupId = device.groupId
        }
        ...
        let deviceInfo: DeviceInfo = {
          ...
          label:
            device.label ||
            (options.noFillLabel ? device.label : 'speaker ' + (result.audioOut.length + 1))
        }
        if (options.groupId) {
          deviceInfo.groupId = device.groupId
        }
      ...
      if (!this.deviceInited) {
        ...
        if (!firstPermDevice) {
          ...
          if (deviceType === 'audioIn') {
            ...
            this.logger.log(
              `摄像头设备：【${newDeviceRecords[deviceType]
          ...
          newDeviceRecords[deviceType].forEach((newDevice) => {
            ...
            const oldDevice = this.deviceHistory[deviceType].find((oldDevice) => {
              return (
                ...
                (oldDevice.deviceId === '' && newDevice.deviceId === 'default')
              )
            })
            if (!oldDevice) {
              this.logger.log(
                `检测到设备新增：${deviceType}: deviceId ${newDevice.deviceId} 【${newDevice.label}】`
              )
              triggers[deviceType].added.push(newDevice)
            } else if (
              oldDevice.label !== newDevice.label ||
              oldDevice.groupId !== newDevice.groupId
            ) {
          ...
          this.deviceHistory[deviceType].forEach((oldDevice) => {
            ...
            if (!newDevice) {
              this.logger.log(
                `检测到设备拔出：${deviceType}: deviceId ${oldDevice.deviceId} 【${oldDevice.label}】`
              )
  ...
  defaultHandleUserGestureNeeded(e: { name: string; message: string }) {
    ...
    this.logger.warn(
      `为避免看到这个消息，您应该实现 Device.onUserGestureNeeded 方法，引导用户作出手势，并触发 Device.on("user-gesture-fired")事件。`
    )
  }
