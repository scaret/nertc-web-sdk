    ...
    if(!this.videoDom) {
      ...
      this.videoDom.addEventListener("resize", (evt)=> {
        if (this.videoDom === evt.target && this.videoRenderMode.width && this.videoRenderMode.height){
          this.adapterRef.logger.log("setVideoRender on resize", this.videoDom?.videoWidth, this.videoDom?.videoHeight);
          this.setVideoRender();
        }
      });
    ...
    if(!this.screenDom) {
      ...
      this.screenDom.addEventListener("resize", (evt)=> {
        if (this.screenDom === evt.target && this.screenRenderMode.width && this.screenRenderMode.height){
          this.adapterRef.logger.log("setScreenRender on resize", this.screenDom?.videoWidth, this.screenDom?.videoHeight);
