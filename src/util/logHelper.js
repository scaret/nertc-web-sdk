...
function logHelper(options = {}) {
	var self = this;

	if(options.maxLogsLines && typeof options.maxLogsLines === 'number' && !isNaN(options.maxLogsLines)) {

	} else {
		options.maxLogsLines = 5000
	}
	...
	// OPTIONS
	self.useTimestamps = options.useTimestamps || false; // insert a timestamp in front of each log
	self.useLocalStorage = options.useLocalStorage || false; // store the output using window.localStorage() and continuously add to the same log each session
	self.autoTrim = true; // to avoid the log eating up potentially endless memory
	self.maxLines = options.maxLogsLines || 5000; // if autoTrim is true, this many most recent lines are saved
	self.tailNumLines = 100; // how many lines tail() will retrieve
	self.logFilename = options.logFilename || 'nimWebRtcLog.txt'; // filename of log downloaded with downloadLog()
	self.maxDepth = 5; // max recursion depth for logged objects
	...
	*/	
	this.getLog = function() {
		...
		if (self.useLocalStorage) {
			var saved = window.localStorage.getItem('nimWebRtcLog');
			if (saved) {
	...
	this.clear = function() {
		...
		if (self.useLocalStorage) {
			...
			window.localStorage.setItem('nimWebRtcLog', saveObject);
		}
	}
	// records a log
